...
!function() {
    ...
    var t = n.decimal,
        ...
        i = r && e ? function(n, t) {
          for (var u = n.length,
              ...
              a = 0,
              o = r[0],
              l = 0; u > 0 && o > 0 && (l + o + 1 > t && (o = Math.max(1, t - l)), i.push(n.substring(u -= o, u + o)), !((l += o + 1) > t)); )
            o = r[a = (a + 1) % r.length];
 ...
 while ((s = (s + l + 4) % 4) !== f);
  ...
  function yr(n, t) {
    ...
    for (n += "", t += ""; (e = hl.exec(n)) && (r = gl.exec(t)); )
      (u = r.index) > i && (u = t.slice(i, u), o[a] ? o[a] += u : o[++a] = u), (e = e[0]) === (r = r[0]) ? o[a] ? o[a] += r : o[++a] = r : (o[++a] = null, l.push({
        i: a,
        x: mr(e, r)
      })), i = gl.lastIndex;
  ...
  function Mr(n, t) {
    for (var e,
        r = oa.interpolators.length; --r >= 0 && !(e = oa.interpolators[r](n, t)); )
      ;
    return e;
  }
  function xr(n, t) {
 ...
 while (!u || u > 1);
  ...
  }), "function" == typeof define && define.amd ? (this.d3 = oa, define("vis/js/d3.js", [], oa)) : "object" == typeof module && module.exports ? module.exports = oa : this.d3 = oa;
}();

})();
System.register("vis/js/GeiVisUtils.js", ["npm:babel-runtime@5.8.38/core-js/number/parse-float.js", "vis/js/d3.js"], function (_export) {
	var _Number$parseFloat, d3;

	function generateTagBundles(entries) {
					...
					tags.forEach(function (tagA) {
						...
						tags.forEach(function (tagB) {
							if (tagA != tagB) {
								...
								var tempLink = [tagA, tagB].sort(function (a, b) {
									if (a < b) {
										return 1;
									} else {
										return -1;
		...
		var r = linkList.keys().map(function (l) {
			...
			var parent = _Number$parseFloat(s[0]);
			return {
				'source': { 'key': _Number$parseFloat(s[0]), 'name': _Number$parseFloat(s[0]), 'parent': parent },
				'target': { 'key': _Number$parseFloat(s[1]), 'name': _Number$parseFloat(s[1]), 'parent': parent },
				'strength': linkList.get(l)
			};
		});

		//console.log({'nodes' : nodes.map(function (n) {return { 'name' : n} ;}), 'links': r });
		var output = { 'nodes': nodes.map(function (n) {
				return { 'name': n };
			}), 'links': r };
		//console.log(output);
		return output;
	...
	function generateLinksAndNodes(entries) {
	...
	return {
		setters: [function (_npmBabelRuntime5838CoreJsNumberParseFloatJs) {
		...
		execute: function () {
			...
			_export("generateTagBundles", generateTagBundles);

			_export("generateLinksAndNodes", generateLinksAndNodes);

			_export("makeSafeForCSS", makeSafeForCSS);
		}
	};
});
System.register("vis/tags2.js", ["vis/js/d3.js", "vis/js/GeiVisUtils.js"], function (_export) {
  // import the modules

  "use strict";

  var d3, GeiVisUtils, diameter, radius, margin;
  ...
  function addTooltip(circle) {
    ...
    var tooltip = d3.select("#plot").append("text").text(text).attr("x", x).attr("y", y).attr("dy", -r * 2).attr("id", "tooltip");
  ...
  function drawGraph(data) {
    ...
    // create svg image
    var svg = d3.select("body").select("#circle").append("svg").attr("width", diameter).attr("height", diameter);
    ...
    var plot = svg.append("g").attr("id", "plot").attr("transform", "translate(" + radius + ", " + radius + ")");
    ...
    circleLayout(graph.nodes.sort(function (a, b) {
  ...
  function circleLayout(nodes) {
    ...
    var scale = d3.scale.linear().domain([0, nodes.length]).range([0, 2 * Math.PI]);
  ...
  function drawNodes(nodes) {
    ...
    d3.select("#plot").selectAll(".node").data(nodes).enter().append("circle").classed("node", true).attr("id", function (d, i) {
      return d.name;
  ...
  function drawCurves(links) {
    ...
    d3.select("#plot").selectAll(".link").data(links).enter().append("path").attr("class", function (d) {
  ...
  return {
    setters: [function (_visJsD3Js) {
      d3 = _visJsD3Js;
    }, function (_visJsGeiVisUtilsJs) {
      GeiVisUtils = _visJsGeiVisUtilsJs;
    }],
    execute: function () {
      diameter = 1000;
      radius = diameter / 2;
...
//# sourceMappingURL=tagBundle.js.map